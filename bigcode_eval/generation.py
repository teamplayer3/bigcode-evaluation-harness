...
from bigcode_eval.utils import TokenizedDataset, complete_code, PreparedPrompts, complete_code_remote
...
class EndOfFunctionCriteria(StoppingCriteria):
    ...
    def __init__(self, start_length, eof_strings, tokenizer, check_fn=None):
        ...
        if check_fn is None:
            def check_fn(decoded_generation): return any(
...
def parallel_generations(task, dataset, accelerator, model, tokenizer, n_tasks, args):
    if args.load_generations_path:
        ...
        with open(args.load_generations_path) as fp:
            ...
            import jsonlines
            generations = list(jsonlines.Reader(fp))
            generations = [g["generation"] for g in generations]
...
def remote_generations(task, endpoint, dataset, n_tasks, args):
    if args.load_generations_path:
        # load generated code
        with open(args.load_generations_path) as fp:
            generations = json.load(fp)
    ...
    # The input_length / start_length set to 0 for now will be adjusted later
    # Check if the task has a custom check_fn method for the stopping criteria
    if task.stop_words and endpoint.tokenizer.eos_token:
        task.stop_words.append(endpoint.tokenizer.eos_token)
    if hasattr(task, "check_fn"):
        stopping_criteria.append(
            EndOfFunctionCriteria(0, task.stop_words,
                                  endpoint.tokenizer, task.check_fn)
        )
    elif task.stop_words:
        stopping_criteria.append(
            EndOfFunctionCriteria(0, task.stop_words, endpoint.tokenizer)
        )
    if hasattr(task, "max_length_multiplier") and task.max_length_multiplier:
        stopping_criteria.append(
            TooLongFunctionCriteria(0, task.max_length_multiplier)
    ...
    generations = complete_code_remote(
        task,
        endpoint,
        prompts,
        n_tasks=n_tasks,
